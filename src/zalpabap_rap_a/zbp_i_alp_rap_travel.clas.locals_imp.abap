...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_alp_rap_travel IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF zi_alp_rap_travel IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
...
*    ENDIF.
    ...
    DATA business_data TYPE TABLE OF zsc_alp_rap_agency=>tys_z_travel_agency_es_5_type.
    ...
    IF agencies IS NOT INITIAL.
      ...
      TRY.
          ...
          NEW zcl_alp_ce_rap_agency( )->get_agencies(
            EXPORTING
...
*      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
      IF travel-AgencyID IS INITIAL OR NOT line_exists( business_data[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_alp_rap(
                                          ...
                                          textid   = zcm_alp_rap=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_alp_rap(
                                          severity   = if_abap_behv_message=>severity-error
                                          textid     = zcm_alp_rap=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_alp_rap(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_alp_rap=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_alp_rap(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_alp_rap=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
                        %element-BeginDate = if_abap_behv=>mk-on )
          TO reported-travel.
      ENDIF.
  ...
  METHOD get_instance_authorizations.
    ...
    "For the Travel Status we need the before-image from the database. We perform this for active
    "(is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
    SELECT FROM zalprap_atrav
...
*                                       requested_authorizations-%action-Prepare      = if_abap_behv=>mk-on OR
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky = travel-%tky
                            %msg = NEW zcm_alp_rap( severity = if_abap_behv_message=>severity-error
                                                    textid   = zcm_alp_rap=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
      ...
      APPEND VALUE #( %tky                 = travel-%tky
                      ...
                      %action-acceptTravel = COND #( WHEN update_granted = abap_true
                                                     THEN if_abap_behv=>auth-allowed
                                                     ELSE if_abap_behv=>auth-unauthorized )
                      %action-rejectTravel = COND #( WHEN update_granted = abap_true
                                                     THEN if_abap_behv=>auth-allowed
                                                     ELSE if_abap_behv=>auth-unauthorized )
...
*                                                     ELSE if_abap_behv=>auth-unauthorized )
                      %action-Edit         = COND #( WHEN update_granted = abap_true
                                                     THEN if_abap_behv=>auth-allowed
                                                     ELSE if_abap_behv=>auth-unauthorized )
                      %assoc-_Booking      = COND #( WHEN update_granted = abap_true
                                                     THEN if_abap_behv=>auth-allowed
                                                     ELSE if_abap_behv=>auth-unauthorized )
                      %delete              = COND #( WHEN delete_granted = abap_true
                                                     THEN if_abap_behv=>auth-allowed
                                                     ELSE if_abap_behv=>auth-unauthorized )
                    ) TO result.
    ENDLOOP.
  ...
  METHOD is_create_granted.
    ...
    AUTHORITY-CHECK OBJECT 'ZALPOSTAT'
      ID 'ZALPOSTAT' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    ...
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZALPOSTAT'
        ID 'ZALPOSTAT' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZALPOSTAT'
        ID 'ZALPOSTAT' DUMMY
